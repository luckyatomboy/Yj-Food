...
function check_sub($id){
	$db =& MysqlConnection::get();
    	$sql = 'select * from '.Config::$tbl_prefix.'product_categories where product_category_id='.intval($id);
    	$res = $db->query($sql);
...
<form name="form1" id="form1" action="index.php?_m=mod_product&_a=save_move" method="post">
		<tr>
			<td width="30%">
			<input type="hidden" name="product" value="<?php echo $product;?>" />
			<?php _e('Choose product category'); ?>
			</td>
