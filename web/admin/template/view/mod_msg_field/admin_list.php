...
function delete_field(field_id) {
	if (confirm("<?php _e('Delete the selected field?'); ?>")) {
		$.get(parseToAdminURL('mod_msg_field',"admin_delete",{field_id:field_id}), function(response) {
...
$(function(){
	$('#admin_field_list').sortable({
		...
		stop:function(e,ui){
			...
			$.ajax({
				 type: "POST",
				 url: parseToAdminURL('mod_msg_field',"admin_order"),
				 data:data,
...
?>
	<li><a class="iconbk nopngfilter_spec" href="<?php echo Html::uriquery('mod_message', 'admin_list'); ?>" title=""><?php _e('Back'); ?></a></li>
<?php
}
?>
 <?php
    if(ACL::isAdminActionHasPermission('mod_msg_field', 'admin_add')){
?>
	<li><a class="msgcont nopngfilter_spec" href="<?php echo Html::uriquery('mod_msg_field', 'admin_add');?>" title=""><?php _e('Add message field'); ?></a></li>
<?php
}
?>
</ul>
...
<ul id="admin_field_list">
    ...
    <tbody>
        ...
        <tr >
					<td width="20%"><?php echo MsgField::getUserDefineLabel($field); ?></td>
		<td width="25%" title="<?php _e('You can drag the field to the corresponding position'); ?>" style="cursor:move;">
			<?php echo MsgField::showDisplayFieldStyle($field); ?>
		</td>
        	<td width="15%">
        	...
        	<td width="10%" class="img_achor"><?php echo Toolkit::validateYesOrNo($field['showinlist'],"img_showinlist_".$field['id'],Html::uriquery('mod_msg_field', 'admin_pic', array('_id' => $field['id'],'_tag'=>'showinlist')));?></td>
			<td width="10%" class="img_achor"><?php echo Toolkit::validateYesOrNo($field['required'],"img_required_".$field['id'],Html::uriquery('mod_msg_field', 'admin_pic', array('_id' => $field['id'],'_tag'=>'required')));?></td>
        	<td width="10%" class="field_op">
			<?php if($field['field_type']!=0){ ?>
        		<a href="<?php echo Html::uriquery('mod_msg_field', 'admin_edit', array('id' => $field['id']));?>" title="<?php _e('Edit'); ?>">
