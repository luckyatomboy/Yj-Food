...
//        $is_remote_tpl = trim(ParamHolder::get('is_remote', '0'));
    ...
    public function reset_tpl_data($tpl_name = DEFAULT_TPL,$flag = 1) {
		...
		if($this->downloadsql($tpl_name)){
			...
			$db->query($del_sql);
			if(true || !Toolkit::getAgent() &&!IS_INSTALL)
			{
				
				
				$update_qq = "UPDATE {$prefix}online_qqs SET account='2085692859'";
...
SQL;
				$update_friendlinks = "UPDATE {$prefix}friendlinks SET fl_img = 'friend_link.gif',fl_addr='http://mobanlin.com',fl_name='模板林'";
				$update_module_blocks = "UPDATE {$prefix}module_blocks SET s_param = 'a:1:{s:4:\"html\";s:169:\"Copyright &copy 2009-2014,mobanlin.com,All rights reserved<br />版权所有 &copy 企业网站模板 未经许可 严禁复制 建议使用1024X768分辨率浏览本站\";}' where alias=\"mb_foot\" and title = '' ";
				$delet_module_blocks  = "UPDATE {$prefix}module_blocks SET s_param = 'a:1:{s:4:\"html\";s:302:\"<div id=\"com_con_sq\" class=\"com_con_sq\">Power by <a style=\"display:inline;\" title=\"模板林\" target=\"_blank\" href=\"http://mobanlin.com/\">模板林</a>|<a style=\"display:inline;\" title=\"企业网站模板\" target=\"_blank\" href=\"http://mobanlin.com/\">企业网站模板下载</a>&nbsp;版权所有</div>\";}' where alias=\"mb_foot\" and title is NULL ";
				try{
					$db->query($update_qq);
					$db->query($update_siteinfo);
					$db->query($update_moduleblocks);
					$db->query($update_friendlinks);
					$db->query($update_module_blocks);
					$db->query($delet_module_blocks);
				} catch(Exception $ex) {
