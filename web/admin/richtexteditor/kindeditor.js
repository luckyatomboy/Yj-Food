...
K.options = {
	...
	minChangeSize : 50,
	items : [
		'source', '|', 'emoticons','undo', 'redo', '|', 'preview', 'template',
		'plainpaste', 'wordpaste', '|', 'justifyleft', 'justifycenter', 'justifyright',
		'justifyfull', 'insertorderedlist', 'insertunorderedlist', 'indent', 'outdent', 'subscript',
		'superscript', 'clearhtml', 'quickformat', 'selectall','baidumap', 'pagebreak','/',
...
_extend(KWidget, {
	...
	pos : function(x, y, updateProp) {
		...
		if (y !== null) {
			if(parent.document.getElementById("adminiframe")!=null){
				var _adminiframe=parent.document.getElementById("adminiframe");
				var screenHeight=window.screen.availHeight;
				screenHeight=(screenHeight)/2-102;
				y=100;
				if(_adminiframe.src.indexOf("admin_edit")>0){
					y=70;
				}
			}
			if(parent.document.getElementById("showContents")!=null){
				var _showContents2=parent.document.getElementById("showContents").src;
				
				if(_showContents2.indexOf("admin_company_introduction")>0){
					y=40;
				}
			}
			y = y < 0 ? 0 : _addUnit(y);
...
_extend(KDialog, KWidget, {
	init : function(options) {
		...
		if (self.height) {
			_tmp=_removeUnit(self.height) - headerDiv.height() - footerDiv.height();
			if(_tmp>350){
				_tmp = _tmp-100;
			}
			bodyDiv.height(_tmp);
		...
		_t_height=self.div.height();
		if(_t_height>410){
			_t_height=_t_height-100;
		}
		self.div.height(_t_height);
		self.mask = null;
		if (showMask) {
			var docEl = _docElement(self.doc),
