 ...
 * 
 */
class MsgField extends RecordObject {
    ...
    protected $yes_validate = array(
		...
		public static function findAll2($where='',$orderby=''){
				...
				$return = $row = array();
				$table_name= Config::$tbl_prefix.'msg_fields';
		...
		$fieldtype=$fieldinfo['field_type'];
			switch ($fieldtype) {
				case 3:
					...
					$opts_data_hash=array();
					foreach($opts_data as $onedata){
						$opts_data_hash[$onedata['key']]=$onedata['val'];
					}
					foreach($fieldval as $eachval){
						$resval=self::getMultilangLabel($opts_data_hash[$eachval]);
						if(!empty($resval)) $resvalarr[]=$resval;
					}
					$fieldval=implode(',', $resvalarr);
				break;
				case 4:
					$resvalarr=array();
					$field_option = unserialize($fieldinfo['options']);
					...
					$resval=self::getMultilangLabel($opts_data_hash[$fieldval]);
					if(!empty($resval)) $resvalarr[]=$resval;
					$fieldval=implode(',', $resvalarr);
				break;
	...
	public static function getUserCustomComponent($fieldinfo,$paramarr,$userinfo) {
		...
		switch ($fieldtype) {
			...
			case 2:
				if($is_only_display){  
					$comp='<textarea name="extends['.$fieldname.']"  rows="3" cols="15" readOnly >'.$fieldval.'</textarea> ';
				}
				else{
			...
			case 5:
				if($is_only_display){
					$comp=Html::input('text', 'mess['.$propname.']',__('Date picker'),  ' readOnly');
...
_addDatePicker($('input[name=\"extends[{$fieldname}]\"]'));			
	...
	public static function getUserDefaultFieldComponent($fieldinfo,$userinfo){
				...
				if($fieldinfo['required']=='1') $validatestr=' required ';
				switch ($propname) {
						case 'username':case 'email':case 'tele':
								if($is_only_display){
									$comp=Html::input('text', 'mess['.$propname.']',$val,  'readOnly');
								}else{
									$comp=Html::input('text', 'mess['.$propname.']',$val,  'class="textinput" '.$validatestr);
								}
								break;
						case 'message':
								if($is_only_display){
									$comp=Html::textarea('mess[message]', '', 'rows="3" cols="15" readOnly '.$validatestr);
								}else{
									$comp= Html::textarea('mess[message]', '', 'rows="3" cols="10" '.$validatestr);
								}
								break;
						case 'gender':
								$cur_lang=$fieldinfo['_cur_lang'];
								 if(empty($cur_lang)) $cur_lang=SessionHolder::get('_LOCALE', '');
								if($is_only_display){
									$comp.="<input name=mess[{$propname}] type='radio' value='M' readOnly/> ".__('Male');
									$comp.="  <input name=mess[{$propname}] type='radio' value='F' readOnly/> ".__('Female');
								}else{
									$comp.="<input name=mess[{$propname}] class='fieldtype4' type='radio' value='M' ".($val=='M'?'checked':'')." {$validatestr}/> ".__('Male');
									$comp.="  <input name=mess[{$propname}] class='fieldtype4' type='radio' value='F' ".($val=='F'?'checked':'')." {$validatestr}/> ".__('Female');
								}
								break;
						case 'birthday':
								if($is_only_display){
								...
								$comp=Html::input('text', 'mess['.$propname.']',!empty($val)&&$val!='0000-00-00'?$val:'',  'style="width:100px;" class="textinput  date" '.$validatestr);
							$comp.="<script>
$(function(){
_addDatePicker($('input[name=\"mess[{$propname}]\"]'));			
});
</script>";
								}
								break;
	...
	public static function getMaxOrder() {
        $db =& MySqlConnection::get();
        $sql = "SELECT MAX(i_order) AS max_order FROM ".Config::$tbl_prefix."msg_fields";
        $rs =& $db->query($sql);
