...
class ModArticle extends Module {
    ...
    public function fullist() {
        ...
        $tit_res = $o_block->find(" `module`=? and `action`=? and  s_locale=? and s_query_hash=?",array("mod_article","recentarticles",$curr_locale,$link_mod)," limit 1"); 
        if (!$tit_res) {
        	$tit_res = $o_block->find(" `module`=? and `action`=? and  s_locale=? and s_query_hash=?",array("mod_article","recentarticles",$curr_locale,$link_all)," limit 1"); 
        	if (!$tit_res) {
        		$tit_res = $o_block->find(" `module`=? and `action`=? and  s_locale=? ",array("mod_article","recentarticles",$curr_locale)," limit 1"); 
        	}
        }
        $search_where = '';
        $search_params = array();
...
//                }         
			...
			if(!$more){
				...
				$this->assign('mod_title',$tit_res->title);
			}
